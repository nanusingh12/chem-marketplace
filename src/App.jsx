...
const staticBlogPosts = {
  trends: [
    { id: 1, title: 'Green Chemistry Revolution: Sustainable Alternatives Gaining Market Share', excerpt: 'Bio-based chemicals projected to capture 25% of market by 2025 as companies prioritize sustainability.', image: '🌿', category: 'Sustainability', date: '2024-01-15', readTime: '4 min read', author: 'Dr. Sarah Chen', authorRole: 'Sustainability Expert', trending: true, views: '2.4K' },
    { id: 2, title: 'Digital Transformation in Chemical Supply Chain', excerpt: 'Blockchain and AI are revolutionizing chemical logistics and quality control processes.', image: '🤖', category: 'Technology', date: '2024-01-12', readTime: '6 min read', author: 'Mike Rodriguez', authorRole: 'Tech Analyst', trending: true, views: '1.8K' }
  ],
  innovations: [
    { id: 1, title: 'Breakthrough in Catalyst Technology Boosts Efficiency', excerpt: 'New catalyst design reduces energy consumption by 40% in key industrial processes.', image: '⚡', category: 'Innovation', date: '2024-01-10', readTime: '5 min read', author: 'Dr. Emily Watson', authorRole: 'Research Director', trending: false, views: '1.2K' }
  ],
  market: [
    { id: 1, title: 'Asia-Pacific Chemical Market Growth Outlook 2024', excerpt: 'China and India lead regional expansion with 8.2% projected growth rate.', image: '📊', category: 'Market Analysis', date: '2024-01-08', readTime: '7 min read', author: 'James Kim', authorRole: 'Market Analyst', trending: false, views: '980' }
  ]
};
        ...
        <div>
          ...
          <ul style={{ listStyle: 'none' }}>
            {[{ name: t.aboutUs, action: () => alert('About ChemMarket: Leading chemical marketplace since 2024!') }, { name: t.careers, action: () => alert('Career opportunities coming soon!') }, { name: t.pressKit, action: () => alert('Press kit available upon request.') }, { name: t.sustainability, action: () => alert('Sustainability initiatives coming soon!') }, { name: t.partners, action: () => alert('Partnership program details coming soon!') }].map((item, index) => (
              <li key={index} style={{ marginBottom: 'var(--space-3)' }}><button onClick={item.action} style={{ background: 'none', border: 'none', color: 'var(--gray-4D400)', fontSize: 'var(--text-base)', transition: 'all 0.3s var(--ease-out)', display: 'block', padding: 'var(--space-1) 0', cursor: 'pointer', textAlign: 'left', width: '100%' }} onMouseEnter={(e) => { e.target.style.color = 'var(--primary-300)'; e.target.style.transform = 'translateX(8px)'; }} onMouseLeave={(e) => { e.target.style.color = 'var(--gray-400)'; e.target.style.transform = 'translateX(0)'; }}>🏢 {item.name}</button></li>
            ))}
          </ul>
        </div>
      </div>
